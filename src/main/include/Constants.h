...
namespace Constants
{
    ...
    namespace DriveTrain
    {
        ...
        constexpr double
            turnGearing = 72.0 / 14 * 24 / 12, // maybe use std::ratio
            ticksPerRotation = turnGearing * 2048,
            velPerRPS = ticksPerRotation /
                        10, // velocity per rotation/sec = accel per rotation/sec^2
            ticksPerRadian =
                ticksPerRotation / 2 / pi;                      // todo: check this with hardware
        constexpr int absSubtraction[]{2220, 2148, 2695, 1023}; // to align the wheels
        constexpr double perSecond =
                             4, // inversely proportional to the intended response time
            maxVel = perSecond * velPerRPS,
                         maxAccel =
                             perSecond * perSecond * velPerRPS;     // for Motion Magic
    ...
    namespace Arm
    {
        ...
        constexpr double momentStage1 = len1 /*in*/ * 14.5 /*lb*/,
                         nAngleStage1 = 0 / 360,
                         momentStage2 = len2 /*in*/ * 9.3 /*lb*/,
                         nAngleStage2 = -0 / 360,
        ...
        // constexpr double initialCorrections[] = {0 * ticksPerRotation * gear1to2 /
        // 360, 10 * ticksPerRotation / 360, 10 * ticksPerRotation / 360};
        //  limits of motion, per stage
        constexpr double minDegrees[] = {-120 * ticksPerRotation * gear1to2 / 360,
                                         15 * ticksPerRotation / 360,
                                         15 * ticksPerRotation / 360},
                         maxDegrees[] = {120 * ticksPerRotation * gear1to2 / 360,
                                         350 * ticksPerRotation / 360,
                                         350 * ticksPerRotation / 360};
    } // namespace Arm
