...
#include "armVectors.h"

class testarmraise : public /* frc2::CommandHelper< */ frc2::
                         WaitCommand /* , testarmraise> */
{
public:
  testarmraise(Arm &a, const double (&angles)[3],
               units::second_t timeout = 15_s)
      : WaitCommand(timeout), arm(a), angs(angles)
  {
    AddRequirements(&a);
  }
...
{
  // return false;
  return frc::DriverStation::IsDisabled() || (timesInRange >= minTimesInRange);
}

struct IntCtl : public frc2::WaitCommand
{
  ...
  void End(bool) override { it.SetDeployed(Stop); }
  Intake &it;
...
frc2::Command *RobotContainer::GetAutonomousCommand()
{
  ...
  case 2:
    return new frc2::SequentialCommandGroup(
        testarmraise(arm, openInFront), testarmraise(arm, highPost),
        IntCtl(intake, ConeIn), testarmraise(arm, openInFront),
...
{ // todo more!!
  if (XBox)
  {
    ...
    if (lefTrig < stickError)
      return baseThrottle + righTrig * (maxThrottle - baseThrottle);
    else if (righTrig < stickError)
      return baseThrottle + lefTrig * (minThrottle - baseThrottle);
    else
      return baseThrottle;
  }
  return (maxThrottle + minThrottle -
          drivestickJ.GetThrottle() * (maxThrottle - minThrottle)) /
         2;
}
#ifdef FdCtrTog
...
{
  if (XBox)
    return driverstick.GetRightBumperPressed();
...
{
  if (XBox)
    return driverstick.GetAButtonReleased();
...
void RobotContainer::DrvbyStick::Execute()
{
...
#ifdef FdCtrTog
  if (bot.GetFieldCenterToggle())
    fieldcentered ^= true;
#endif
  if (bot.GetZeroYaw())
    it.ZeroYaw();
  // todo: add throttle
  double linX = -bot.GetX(), linY = bot.GetY(), rot = bot.GetRot();
  // Dead zones
  ...
  it.SetV(linX, linY, rot, bot.GetThrottle(), fieldcentered);
}
